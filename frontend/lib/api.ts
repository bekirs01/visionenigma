...
export const api = {
  ...
  /** XLSX export: fetch with credentials, then download as blob */
  async exportXlsxDownload(params?: { search?: string; status?: string; category_id?: number }): Promise<void> {
    const sp = new URLSearchParams();
    if (params?.search) sp.set("search", params.search);
    if (params?.status) sp.set("status", params.status);
    if (params?.category_id != null) sp.set("category_id", String(params.category_id));
