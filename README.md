...
Чувствительные данные (ключи API, пароли БД, коды доступа) хранятся только в локальных файлах `.env`; эти файлы в репозиторий не попадают (см. `.gitignore`). В репозитории лежат лишь шаблоны `.env.example` с пустыми или подставными значениями. При развёртывании на сервере обычно используют переменные окружения платформы или секреты CI/CD, не файлы `.env` из репозитория.
...
## 8. Очистка истории Git от случайно закоммиченных секретов

Если файл с секретами (например `backend/.env`) когда-либо попадал в коммиты, удаление его из текущего дерева недостаточно: он остаётся в истории. Ниже — способ переписать историю и убрать такой файл из всех коммитов. **После перезаписи истории потребуется force-push** (`git push --force`); все, кто клонировал репозиторий, должны будут переклонировать или перебазировать свои ветки.

**Важно:** команды ниже не выводят и не подставляют реальные значения секретов; они только удаляют указанный файл из истории.

1. Установить [git-filter-repo](https://github.com/newren/git-filter-repo) (рекомендуется) или использовать BFG Repo-Cleaner.

2. Удалить файл из всей истории (пример для `backend/.env`):
   ...
   Либо с BFG (файл остаётся в рабочей копии, из истории удаляется):
   ...
   Для BFG путь может потребоваться уточнить (например `backend/.env` в зависимости от версии BFG).

3. Добавить remote заново (после `git filter-repo` remote удаляется):

   ```bash
   git remote add origin https://github.com/bekirs01/visionenigma.git
   ```

4. Отправить переписанную историю (только если вы уверены в результате и согласовали с участниками):

   ```bash
   git push --force origin main
   ```

   При необходимости выполнить то же для других веток (например `support-mvp`).

Локальный файл `backend/.env` при этом не удаляется; он остаётся только на вашей машине и в репозиторий больше не попадёт, если он добавлен в `.gitignore` и с индекса снят (`git rm --cached backend/.env`).

---

## 9. Лицензия и примечания
