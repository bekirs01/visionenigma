...
class HistorySearchResult:
    """Результат поиска в истории обращений"""
    ticket_id: int
    subject: str
    body_snippet: str
    issue_summary: str
    ...
    score: float
    similarity: float  # Нормализованная похожесть 0-1


class HistorySearchService:
    """
    Сервис поиска похожих обращений в истории.
    Позволяет AI использовать прошлые хорошие ответы вместо генерации с нуля.
    """

    # Порог похожести для использования прошлого ответа
    ...
    def search_similar_tickets(
        ...
        """
        Поиск похожих обращений в истории.
        ...
        Returns:
            Список похожих тикетов с ответами
        """
        from app.models import Ticket

        # Ищем только завершённые тикеты с хорошими ответами
        tickets_query = self.db.query(Ticket).filter(
            Ticket.reply_sent == 1,  # Ответ был отправлен
            Ticket.ai_reply.isnot(None),  # Есть AI ответ
            Ticket.ai_reply != "",
        )

        if category:
            tickets_query = tickets_query.filter(
                Ticket.request_category == category
            )

        # Берём последние 500 для эффективности
        tickets = tickets_query.order_by(Ticket.id.desc()).limit(500).all()

        if not tickets:
            return []

        query_lower = query.lower()
        query_tokens = self.kb_service._tokenize(query_lower)

        scored_tickets = []
        for ticket in tickets:
            # Считаем релевантность
            ticket_text = f"{ticket.subject or ''} {ticket.body or ''} {ticket.issue_summary or ''}"
            score = self.kb_service._calculate_score(
                query_lower,
                query_tokens,
                ticket.subject or "",
                ticket_text,
                ticket.request_category or ""
            )

            if score > 0:
                # Нормализуем в похожесть 0-1
                max_possible_score = len(query_tokens) * 3 + 20  # Примерная оценка максимума
                similarity = min(score / max_possible_score, 1.0)

                snippet = self.kb_service._extract_snippet(ticket.body or "", query_tokens, 150)

                scored_tickets.append(HistorySearchResult(
                    ticket_id=ticket.id,
                    subject=ticket.subject or "",
                    body_snippet=snippet,
                    issue_summary=ticket.issue_summary or "",
                    ai_reply=ticket.ai_reply or "",
                    request_category=ticket.request_category or "",
                    score=score,
    ...
    def get_best_matching_reply(
        self,
        query: str,
        category: Optional[str] = None
    ) -> Optional[HistorySearchResult]:
        """
        Получить лучший подходящий ответ из истории.

        Returns:
            HistorySearchResult если найден подходящий, иначе None
        """
        results = self.search_similar_tickets(query, category, top_k=1)
    ...
    def get_history_context_for_llm(
        self,
        query: str,
        category: Optional[str] = None,
        top_k: int = 2
        ...
        """
        Формирует контекст из истории обращений для LLM.
        """
        results = self.search_similar_tickets(query, category, top_k)

        if not results:
            return ""

        context_parts = ["### ПОХОЖИЕ ОБРАЩЕНИЯ ИЗ ИСТОРИИ (используй как основу для ответа):"]

        for i, result in enumerate(results, 1):
            # Убираем персональные данные из ответа
            clean_reply = self._anonymize_reply(result.ai_reply)

            context_parts.append(f"""
--- Обращение {i} (похожесть: {result.similarity:.0%}) ---
Категория: {result.request_category}
Суть: {result.issue_summary}
Ответ поддержки: {clean_reply[:500]}{'...' if len(clean_reply) > 500 else ''}
""")

        return "\n".join(context_parts)

    def _anonymize_reply(self, reply: str) -> str:
        """Удаляет персональные данные из ответа."""
        import re

        # Убираем возможные email
        reply = re.sub(r'[\w\.-]+@[\w\.-]+\.\w+', '[email]', reply)
        # Убираем телефоны
        reply = re.sub(r'\+?[78][\s\-]?\(?\d{3}\)?[\s\-]?\d{3}[\s\-]?\d{2}[\s\-]?\d{2}', '[телефон]', reply)
        # Убираем имена в формате "Иванов И.И."
        reply = re.sub(r'[А-ЯЁ][а-яё]+\s+[А-ЯЁ]\.[А-ЯЁ]\.', '[ФИО]', reply)
...
def search_history(db: Session, query: str, category: Optional[str] = None, top_k: int = 3) -> List[HistorySearchResult]:
    """Поиск похожих обращений в истории."""
    service = HistorySearchService(db)
    return service.search_similar_tickets(query, category, top_k)


def get_history_context(db: Session, query: str, category: Optional[str] = None, top_k: int = 2) -> str:
    """Получить контекст из истории для LLM."""
    service = HistorySearchService(db)
    return service.get_history_context_for_llm(query, category, top_k)
